...
- The server itself can be skipped, and using something like Supabase would be a better option in this case
...
- The server has to be stateless, and all the states should be handled in external services like Postgres, Redis,
  Grafana and Loki so that you can expand the number of the server instances
- Handling the different services is not trivial with docker compose. Kubernetes is better for it (gateway, db
  migrations, other instances like db and metrics)